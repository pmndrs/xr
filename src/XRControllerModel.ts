...
const applyEnvironmentMapIntensity = (envMapIntensity: number, obj: Object3D): void => {
  obj.traverse((child) => {
    if (child instanceof Mesh && 'envMapIntensity' in child.material) {
...
export class XRControllerModel extends Group {
  ...
  setEnvironmentMap(envMap: Texture | null): XRControllerModel {
    ...
    this.envMap = envMap
    applyEnvironmentMap(envMap, this)

    return this
  }

  setEnvironmentMapIntensity(envMapIntensity = 1): XRControllerModel {
    if (this.envMapIntensity === envMapIntensity) {
      return this
    }

    this.envMapIntensity = envMapIntensity
    applyEnvironmentMapIntensity(envMapIntensity, this)
